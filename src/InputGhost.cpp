...
void** ResolveTableEntry(const char* name) {
    if (g_ResolvedEntries.count(name)) return g_ResolvedEntries[name];
...
Uint32 Hooked_SDL_GetMouseState(float* x, float* y) {
  ...
  if (g_Original_SDL_GetMouseState) return g_Original_SDL_GetMouseState(x, y);
  return 0;
}
...
Uint32 Hooked_SDL_GetGlobalMouseState(float* x, float* y) {
  ...
  if (g_Original_SDL_GetGlobalMouseState) return g_Original_SDL_GetGlobalMouseState(x, y);
  return 0;
}
...
Uint32 Hooked_SDL_GetRelativeMouseState(float* x, float* y) {
  ...
  if (g_Original_SDL_GetRelativeMouseState) return g_Original_SDL_GetRelativeMouseState(x, y);
  return 0;
}

void ApplyDynamicHook(const char* name, void* hookFunc, void** originalStore, bool force = false) {
  void** entry = ResolveTableEntry(name);
  if (!entry) return;
  if (*entry == hookFunc) return;
  if (!force && IsLazyWrapper(*entry)) return;
  DWORD oldProtect;
  if (VirtualProtect(entry, sizeof(void*), PAGE_READWRITE, &oldProtect)) {
    if (originalStore && !*originalStore) *originalStore = *entry;
    *entry = hookFunc;
...
void SimulateClick(UINT msg, float normX, float normY, HWND hWnd) {
  ...
  ApplyDynamicHook("SDL_GetMouseState", (void*)Hooked_SDL_GetMouseState, (void**)&g_Original_SDL_GetMouseState, true);
  ApplyDynamicHook("SDL_GetGlobalMouseState", (void*)Hooked_SDL_GetGlobalMouseState, (void**)&g_Original_SDL_GetGlobalMouseState, true);
  ApplyDynamicHook("SDL_GetRelativeMouseState", (void*)Hooked_SDL_GetRelativeMouseState, (void**)&g_Original_SDL_GetRelativeMouseState, true);
  ...
  if (pPush && *pPush) {
    ...
    SDL_GetMouseFocus_t _SDL_GetMouseFocus = pFocus ? (SDL_GetMouseFocus_t)*pFocus : nullptr;
    SDL_GetWindowID_t _SDL_GetWindowID = pWID ? (SDL_GetWindowID_t)*pWID : nullptr;
    SDL_GetTicksNS_t _SDL_GetTicksNS = pTicks ? (SDL_GetTicksNS_t)*pTicks : nullptr;

    void* focus = _SDL_GetMouseFocus ? _SDL_GetMouseFocus() : nullptr;
    SDL_WindowID wid = (focus && _SDL_GetWindowID) ? _SDL_GetWindowID(focus) : 0;
    if (g_ObservedWID != 0) wid = g_ObservedWID;
    g_LastWindowID = wid;
...
void GetViewportInfo(HWND hWnd, int &vpX, int &vpY, int &vpW, int &vpH) {
  RECT rect;
  if (!GetClientRect(hWnd, &rect)) { vpX=0; vpY=0; vpW=1; vpH=1; return; }
  int w = rect.right - rect.left, h = rect.bottom - rect.top;
  if (w <= 0 || h <= 0) { vpX=0; vpY=0; vpW=1; vpH=1; return; }
  float targetAspect = 16.0f / 9.0f, windowAspect = (float)w / (float)h;
  if (windowAspect > targetAspect) { vpW = (int)(h * targetAspect); vpH = h; vpX = (w - vpW) / 2; vpY = 0; }
  else { vpW = w; vpH = (int)(w / targetAspect); vpX = 0; vpY = (h - vpH) / 2; }
}

void NormalizeCoordinates(int x, int y, HWND hWnd, float &outX, float &outY) {
  int vX, vY, vW, vH; GetViewportInfo(hWnd, vX, vY, vW, vH);
  outX = (float)(x - vX) / (float)vW; outY = (float)(y - vY) / (float)vH;
}

void DenormalizeCoordinates(float normX, float normY, HWND hWnd, int &outX, int &outY) {
  int vX, vY, vW, vH; GetViewportInfo(hWnd, vX, vY, vW, vH);
  outX = vX + (int)(normX * vW); outY = vY + (int)(normY * vH);
}

bool IsSimulated() { return false; }
void RenderDebug() {
  static bool forceHook = false;
  ApplyDynamicHook("SDL_PollEvent", (void*)Hooked_SDL_PollEvent, (void**)&g_Original_SDL_PollEvent, forceHook);
  ...
  if (g_LastSimX >= 0) {
      ...
      int x, y; DenormalizeCoordinates(g_LastSimX, g_LastSimY, hWnd, x, y);
