  ...
  if (_renderType != RenderType::None) {
    if (_renderType >= RenderType::D3D9 && _renderType <= RenderType::D3D12) {
      ...
      if (_renderType == RenderType::D3D9) {
        ...
        if ((direct3D9 = ((LPDIRECT3D9(__stdcall *)(uint32_t))(
        ...
        if (direct3D9->GetAdapterDisplayMode(D3DADAPTER_DEFAULT, &displayMode) <
            0) {
          ::DestroyWindow(window);
          ::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
          return Status::UnknownError;
        ...
        if (((long(__stdcall *)(IDXGIAdapter *, D3D10_DRIVER_TYPE, HMODULE,
                                UINT, UINT, DXGI_SWAP_CHAIN_DESC *,
                                IDXGISwapChain **, ID3D10Device **))(
                D3D10CreateDeviceAndSwapChain))(
                adapter, D3D10_DRIVER_TYPE_HARDWARE, NULL, 0, D3D10_SDK_VERSION,
                &swapChainDesc, &swapChain, &device) < 0) {
          ::DestroyWindow(window);
          ::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
          return Status::UnknownError;
        }
        ...
        if (((long(__stdcall *)(IDXGIAdapter *, D3D_DRIVER_TYPE, HMODULE, UINT,
        ...
        ID3D12Device *device;
        if (((long(__stdcall *)(IUnknown *, D3D_FEATURE_LEVEL, const IID &,
                                void **))(D3D12CreateDevice))(
...
kiero::Status::Enum kiero::bind(void *_hooked, void **_original,
                                void *_function) {
  // TODO: Need own detour function
  ...
  if (g_renderType != RenderType::None) {
#if KIERO_USE_MINHOOK
    if (MH_CreateHook(_hooked, _function, _original) != MH_OK ||
        MH_EnableHook(_hooked) != MH_OK) {
      return Status::UnknownError;
    }
#endif
