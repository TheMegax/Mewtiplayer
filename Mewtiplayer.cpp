...
typedef void(__fastcall *ButchBox_Init_t)(ButchBox *self, bool param2);
static ButchBox_Init_t g_origButchBox_Init = nullptr;

typedef bool(__fastcall *ButchBox_TryPlaceCat_t)(ButchBox *self, void *cat);
static ButchBox_TryPlaceCat_t g_origButchBox_TryPlaceCat = nullptr;

typedef void(__fastcall *ButchBox_ConfigCats_t)(ButchBox *self);
static ButchBox_ConfigCats_t g_origButchBox_ConfigCats = nullptr;

typedef void(__fastcall *ButchBox_TryRemoveCat_t)(ButchBox *self, void *cat);
static ButchBox_TryRemoveCat_t g_origButchBox_TryRemoveCat = nullptr;
...
typedef bool(__fastcall *ButchBox_Depart_t)(ButchBox *self);
static ButchBox_Depart_t g_origButchBox_Depart = nullptr;

typedef void(__fastcall *LoadAdventure_t)(LoadAdventureArgs *args);
static LoadAdventure_t g_origLoadAdventure = nullptr;
...
static void Initialize() {
  ...
  // ButchBox::init
  const uintptr_t butchBoxInitRVA = ScanSignature(&mj, g_gameBase, "ButchBox::init",
                                            "48 8B C4 55 53 56 57 41 56 48 8D 68 A1 48 81 EC E0 00 00 00");

  // ButchBox::try_place_cat
  const uintptr_t butchBoxTryPlaceCatRVA = ScanSignature(&mj, g_gameBase, "ButchBox::try_place_cat",
                                                   "48 89 5C 24 10 48 89 74 24 18 48 89 7C 24 20 55 41 54 41 55 41 56 41 57 48 8B EC 48 81 EC 80 00 00 00 4C 8B FA 4C 8B E9 80 B9 F0 00 00 00 00");

  // ButchBox::config_cats
  const uintptr_t butchBoxConfigCatsRVA = ScanSignature(&mj, g_gameBase, "ButchBox::config_cats",
                                                  "48 89 5C 24 08 48 89 6C 24 18 48 89 74 24 20 57 48 83 EC 40 33 DB");

  // ButchBox::try_remove_cat
  const uintptr_t butchBoxTryRemoveCatRVA = ScanSignature(&mj, g_gameBase, "ButchBox::try_remove_cat",
                                                    "48 8B 81 00 01 00 00 45 33 C9 48 39 10 75 03 4C 89 08 48 8B 81 00 01 00 00 48 39 50 08 75 04 4C 89 48 08");

  // ButchBox::depart
  const uintptr_t butchBoxDepartRVA = ScanSignature(&mj, g_gameBase, "ButchBox::depart",
                                              "48 89 4C 24 08 55 53 56 57 41 54 41 55 41 56 41 57 48 8D 6C 24 E8 48 81 EC 18 01 00 00");

  // "LoadAdventure"
  const uintptr_t loadAdventureRVA = ScanSignature(&mj, g_gameBase, "LoadAdventure",
                                             "48 89 5C 24 08 48 89 7C 24 20 55 48 8D 6C 24 C0 48 81 EC 40 01 00 00");

  // "CRTFree"
  const uintptr_t crtFreeRVA = ScanSignature(&mj, g_gameBase, "CRTFree",
  ...
  // Resolve addresses //
  if (crtFreeRVA) {
    const uintptr_t crtHeapAddr = ResolveRIP(g_gameBase + crtFreeRVA + 15, 3, 7);
    g_pCRTHeap = (HANDLE*)crtHeapAddr;
    Overlay::Log("Resolved CRTHeap pointer: %p", (void*)g_pCRTHeap);
  } else {
    Overlay::Log("FAILED to find CRTFree/CRTHeap signature!");
  }
  ...
  if (butchBoxInitRVA) {
    mj.InstallHook(butchBoxInitRVA, 20,
      (void *)Hook_ButchBox_Init,
      (void **)&g_origButchBox_Init, 10, MOD_NAME);
  } else {
    Overlay::Log("Failed to find ButchBox::init!");
  }

  if (butchBoxTryPlaceCatRVA) {
    mj.InstallHook(butchBoxTryPlaceCatRVA, 27,
      (void *)Hook_ButchBox_TryPlaceCat,
      (void **)&g_origButchBox_TryPlaceCat, 10, MOD_NAME);
  } else {
    Overlay::Log("Failed to find ButchBox::try_place_cat!");
  }

  if (butchBoxDepartRVA) {
    mj.InstallHook(butchBoxDepartRVA, 29,
      (void *)Hook_ButchBox_Depart,
      (void **)&g_origButchBox_Depart, 10, MOD_NAME);
  } else {
    Overlay::Log("Failed to find ButchBox::depart!");
  }

  if (butchBoxConfigCatsRVA) {
    mj.InstallHook(butchBoxConfigCatsRVA, 20,
      (void *)Hook_ButchBox_ConfigCats,
      (void **)&g_origButchBox_ConfigCats, 10, MOD_NAME);
  } else {
    Overlay::Log("Failed to find ButchBox::config_cats!");
  }

  if (butchBoxTryRemoveCatRVA) {
    mj.InstallHook(butchBoxTryRemoveCatRVA, 35,
      (void *)Hook_ButchBox_TryRemoveCat,
      (void **)&g_origButchBox_TryRemoveCat, 10, MOD_NAME);
  } else {
    Overlay::Log("Failed to find ButchBox::try_remove_cat!");
  }

  if (loadAdventureRVA) {
    mj.InstallHook(loadAdventureRVA, 23,
      (void *)Hook_LoadAdventure,
      (void **)&g_origLoadAdventure, 10, MOD_NAME);
  } else {
    Overlay::Log("Failed to find LoadAdventure!");
  }
}
